...
class TestJoinRallyDetailLoad:
    """Test the jr_detail_load fix: real depart.png check + fail-fast."""

    def test_detail_load_uses_real_condition(self):
        """Verify the jr_detail_load timed_wait condition checks for
        depart.png (not lambda:False). Uses source code inspection."""
        import inspect
        from actions import rallies

        # Read the source of join_rally and verify that jr_detail_load
        # uses a real condition (finds "depart.png" in its lambda)
        source = inspect.getsource(rallies.join_rally)

        # Find the jr_detail_load timed_wait call
        lines = source.split('\n')
        jr_line_idx = None
        for i, line in enumerate(lines):
            if '"jr_detail_load"' in line:
                jr_line_idx = i
                break

        assert jr_line_idx is not None, "jr_detail_load not found in join_rally source"

        # Look at the surrounding context (the timed_wait call)
        context = '\n'.join(lines[max(0, jr_line_idx - 5):jr_line_idx + 1])

        # Should reference depart.png (real condition), not lambda: False
        assert "depart.png" in context, \
            f"jr_detail_load should check for depart.png, got:\n{context}"
        assert "lambda: False" not in context, \
            f"jr_detail_load should NOT use lambda:False, got:\n{context}"
    ...
    def test_detail_fail_backs_out_quickly(
        ...
        """When detail screen doesn't load, should back out instead of
        wasting 6s in the polling loop."""
        mock_screenshot.return_value = _dummy_screen()
        mock_template.return_value = _template_shape()

        # Rally icons and join buttons found
        mock_find_all.side_effect = lambda s, name, **kw: {
            "rally/join.png": [(800, 440)],
        }.get(name, [(100, 400)])

        # find_image always returns None (depart never found)
        mock_find.return_value = None

        with patch("actions.rallies.timed_wait") as mock_tw:
            # jr_detail_load returns False (depart not found)
            ...
            # Should have backed out — tap_image("close_x.png") or similar
            # The key assertion: we should NOT see a 6-second polling loop
            # (no time.sleep(0.5) calls in a tight loop)
            assert result is False or result == "lost" or result is None
